...
module.exports = {
  ...
  messageProcessHandler: function messageHandler(m) {
    ...
    if (m.type === 'app-emit') {
      ...
      // face, car, etc
      var visionProto = Matrix.service.protobuf.vision.vision.build('admobilize_vision');
      var i = 0;
      Matrix.service.detection.setupLocal( m.payload.name, function localListener(dp){

        console.log('got message!')

        var result;
          try {
            var result = visionProto.VisionResult.decode(dp.slice(m.payload.name.length))
          } catch (e) {
            console.error('Detection ProtoError', e)
          }

          // console.log('>>>>>', result);
          // console.log('>>>>>>>>', result.rect_detection[0])


          var image = result.rect_detection[0].image.buffer.slice(result.rect_detection[0].image.offset, result.rect_detection[0].image.limit );
          require('fs').writeFileSync('NaN' + i + '.jpg', image)
          i++;

        var options = {
          // image: require('fs').readFileSync('./test/stdtestimage.jpg'),
          // image: result.rect_detection[0].image,
          image: image,
          detection: [ m.payload.name.toUpperCase() ]
        }

        Matrix.events.emit('detection', { appName: 'face', payload: result.rect_detection[0].location })

      //   Matrix.service.detection.process( options, function( err, d ){
      ...
      debug('[M]->', m.payload);
      Matrix.component.led.update( m.payload );
    } else {
