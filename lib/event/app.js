...
module.exports = {
  ...
  messageProcessHandler: function messageHandler(m) {
    ...
    if (m.type === 'app-emit') {
      ...
      // policy lookup
      if (  !_.has(appPolicy.service, m.name) || appPolicy.service[ m.name ] === false ){
        console.error(appName.grey, 'application not configured for', m.name.blue, 'service');
      } else {
        Matrix.events.emit('service-init', m);
      }
    } else if (m.type === 'sensor-init') {
      // policy lookup
      if (  !_.has(appPolicy.sensors, m.name) || appPolicy.sensors[ m.name ] === false ){
        console.error(appName.grey, 'application not configured for', m.name.blue, 'sensor');
      } else {
