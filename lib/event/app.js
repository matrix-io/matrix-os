...
function handleDeviceEvent(event) {
  ...
  if (_.isString(payload)) {
    // support for ifttt and other string payloads
...
module.exports = {
  ...
  closeProcessHandler: function (code, number) {
    debug('App Close:', code, number);
  },
  exitProcessHandler: function (code, number, app) {
    debug('App Exit:', name, code, number);
    ...
    if (_.has(app, 'process.handlers') && !_.isEmpty(name)) {
      Matrix.events.removeListener('app-message', app.process.handlers.crosstalk);
      Matrix.events.removeListener('app-' + name + '-message', app.process.handlers.crosstalk);
      console.log('Event Listeners Removed', name);
    }
  ...
  messageProcessHandler: function messageHandler(m) {
    ...
    if (m.type === 'app-emit') {
      ...
      // fast track to data cache - write on next writeInterval
      if (_.isArray(m.payload)) {
        Matrix.sendCache = Matrix.sendCache.concat(m.payload);
        debug('>>>> array app emit');
        return;
      }
      ...
      if (_.has(process.env, 'CHECK_SERVICES') && appRecord.validInitServiceCmds.indexOf(m.name) === -1) {
        ...
        if (m.hasOwnProperty('serviceType')) {
          m.type = m.serviceType;
        }
        m.appName = appName;
        // send app name so we can emit errors back to app
    ...
    } else if (m.type === 'update-setting') {
      //updating app settings
      appRecord.changeSetting(m.settings);
    } else {
