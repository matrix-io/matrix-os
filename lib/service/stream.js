...
var writeSendCacheInterval = setInterval(function () {
  if (Matrix.sendCache.length > 0) { // do we have a cache or not?
    ...
    var data = _.map(Matrix.sendCache, 'payload'); // we have a cache, enough time has elapsed to write it, data = [ payloads ]
    // only app-eimts are cached
...
function socketEmit(channel, payload) {
  ...
  if (socket && socket.connected && socket.registered) {
    ...
    Matrix.sendCache.push({ channel: channel, payload: payload });
  }
}