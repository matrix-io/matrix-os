...
function stopSocketWatch() {
  ...
  clearInterval(Matrix.socketWatcher);
}
...
function checkStreamingServer(cb) {
  ...
  require('net').connect({
    ...
    //start watcher if this is the first, or if stopped
    if (_.isUndefined(Matrix.socketWatcher) || _.isNull(Matrix.socketWatcher[0])) {
...
function initSocket(cb) {
  ...
  socket.on('open', function() {
    ...
    //TODO: Need to restart all apps to resend config !!!
