...
function Application(name, cb){
    ...
    fb.app.get( name, function(err, resp){
      ...
      if ( _.isNull(resp)){
        ...
        if ( resp.validated ){
          ...
          self.policy = resp.policy;
          self.meta   = resp.meta;
          self.local = false;
          // should provide appId
          self.id     = resp.key;
        } else {
          // populate config
          console.warn('Unvalidated config stored in firebase')
          // self.config = configHelper.validate(resp.config);
        }
      }
  ...
  self.watchForUpdates = function(cb){
    fb.app.watchConfig(self.id, function(){
      // refresh config
      self.getConfig(cb);
    })
  }

  self.watchRuntime = function(cb){
    fb.app.watchRuntime(self.id, function (data) {
      debug('runtime>', data);
      cb(data)
    })
  }
