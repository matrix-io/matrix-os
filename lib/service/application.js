...
// Go through list of integrations and activate
  function activateIntegrations( integrations ){
    ...
    if (!_.isUndefined(integrations)){
      if (integrations.indexOf('zigbee') !== 1) Matrix.device.drivers.zigbee.activate();
      if (integrations.indexOf('zwave') !== 1) Matrix.device.drivers.zwave.activate();
    }
  }
