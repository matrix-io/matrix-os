...
function startApp(name, cb) {
  ...
  async.series([
    ...
    function startAppProcess(cb){
      ...
      if ( process.env.DOCKER_APPS === true){
        try {
          ...
          if ( dockerImages.indexOf('matrix-apphost') === -1 ){
            ...
            proc.execSync('docker run -v '+pwd+'/apps:/apps -it matrix-apphost bash')
          }
          // fire up docker image
          app.process = proc.spawn('docker', ['run', '-v', pwd+'/apps:/apps', '-it', 'matrix-apphost', 'node apps/'+name+'.matrix/index.js']);
