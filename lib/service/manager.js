...
function startApp(name, cb) {
  ...
  try {
    ...
    appConfig = JSON.parse(fs.readFileSync('./apps/' + name + '.matrix/config.json'));
  } catch (e) {
    if (_.isFunction(cb)){
  ...
  _.extend(child, {
    name: name,
    // todo add sensors from config
    sensors: _.chain( appConfig.dashboard ).map(function(r){
  ...
  Matrix.activeApplications.push(child);
  ...
  child.on('message', function(m) {
    ...
    if (m.type === 'app-emit') {
      m.payload.type = m.payload.data.type;
      delete m.payload.data.type;
      ...
      if ( child.sensors.indexOf( m.name ) === -1 ){
        console.error(child.name.grey, 'application not configured for', m.name.blue, 'sensor');
      } else {
