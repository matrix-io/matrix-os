...
function installApp(options, cb) {
  ...
  var tmpFolder = os.tmpdir() + '/matrix_app';
  var updateFile = tmpFolder + '/' + name + '-' + version + Math.round(Math.random() * 100000) + '.zip';
  var targetDir = Matrix.config.path.apps + '/' + name + '.matrix/';

  if (!fs.existsSync(tmpFolder)) fs.mkdirSync(tmpFolder); //If missing apps folder create it
...
function startApp(name, cb) {
  ...
  async.series([
    ...
    function startAppProcess(cb) {
      ...
      // since child process forks from parent, we need to change some things

      // don't pass along debug message
