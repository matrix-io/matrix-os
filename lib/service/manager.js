...
function startApp(name, cb) {
  ...
  child.on('message', function(m) {
    ...
    if (m.type === 'app-emit') {
      ...
      // update local sensor information
      _.each(Matrix.activeApplications, function(app){
        // name from parent scope
        if (app.name === name){
          // add to sensor
          app.sensors = m.payload.sensors;
          debug('Update ActiveApplications sensor: ', app);
        }
      })
...
function stopApp(id, cb) {
  ...
  if (id === 'all-applications'){
    ...
    //TODO: remove sensors that aren't being used by others

    //get list of sensors this app uses
    var deadSensors = _.find(Matrix.activeProcesses, { name: id, type:'app' }).sensors;
