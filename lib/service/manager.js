    ...
    function startAppProcess(cb){
      ...
      if ( process.env.hasOwnProperty('DOCKER_APPS') ){
        ...
        try {
          ...
          // HACK: force stop of container
          proc.execSync('docker stop '+ name);

          // fire up docker image
          app.process = proc.spawn('docker', ['run', '-v', pwd+'/apps:/apps', '--rm', "-l",
           name, '-a', 'stdin', '-a', 'stdout', '-a','stderr','matrix-apphost',
...
function stopApp(id, cb) {
  ...
  if (id === 'all-applications'){
    ...
    if ( applicationInstance.process.docker === true ){
