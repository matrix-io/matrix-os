...
function setupAppListeners(name, cb) {
  console.log('app('.green + name.green + ')->listener'.green);
...
function killAllApps() {
  ...
  console.log('All active apps killed:', ids.join(' '));
}
...
function startApp(name, cb) {
  ...
  if (_.find(Matrix.activeProcesses, { name: name })) {
    return console.error('Cannot start another instance of :', name)
  }
  ...
  var appConfig = JSON.parse( require('fs').readFileSync('./apps/' + name + '.matrix/config.json').toString() );
  ...
  child.on('message', function(m) {
    // console.log('app('.green+name.green+')'.green, m);
    if (m.type === 'app-emit') {
      // Reroutes to dataHandler here
      //TODO: rewrite m.type to have all sensors
      m.appName = appConfig.name.toLowerCase();
      m.appVersion = appConfig.version || 0;
      ...
      Matrix.events.emit('app-config', _.extend(m, {name: name}));
...
function stopApp(id, cb) {
  ...
  if (parseInt(id) === id) {
    ...
    Matrix.events.removeListener('app-' + name + '-message');
    // log('stop %s', id);
    ...
    Matrix.db.service.find({
      ...
      Matrix.db.service.remove({
        ...
        console.log('Removed', num, 'application records');
      });
      if (cb) cb();
    });
  }
