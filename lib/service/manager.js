  ...
  r(appUrl).pipe(fs.createWriteStream(updateFile))
    ...
    .on('close', function() {
      ...
      extract.on('close', function () {
          ...
          if ( !fs.statSync('./apps/' + name + '.matrix/package.json')){
            error(name, ' does not have a','package.json'.blue, 'file');
            return cb('No package.json file found!');
          } else {
            try {
              console.log('npm install')
              var pJson = require('child_process').execSync('npm install', { cwd: './apps/' + name + '.matrix/' });
...
function startApp(name, cb) {
  ...
  if ( !fs.statSync('./apps/' + name + '.matrix/config.json')){
    ...
    try {
      ...
      return cb('Invalid JSON for ', name);
    }
  }
  ...
  child.on('message', function(m) {
    ...
    if (m.type === 'app-emit') {
      ...
      Matrix.events.emit('trigger-out', m);
