...
module.exports = {
  ...
  setupLocal: function(algo, cb){
    ...
    sock.on('message', function(message) {
      try {
        vision_result = messages.VisionResult.decode(message.slice(interest.length))
        // console.log(vision_result)
        // Checking the first detection only!
        console.log('Objects of type ', interest, 'at ', vision_result.rect_detection[0].location)
      } catch (err) {
        console.log(err)
      }
    });
