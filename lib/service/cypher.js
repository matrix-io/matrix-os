...
module.exports = {
  init: function () {
    if (!_.isEmpty(Matrix.deviceSecret)) key = _.pad(Matrix.deviceSecret.substring(0, keyLength), keyLength, 'f00e1337');
    else console.error('Unable to initialize cypher'.yellow);
  },

  encrypt: function(text, prefix) {
    if (_.isUndefined(text) || text === '') return '';
    if (_.isUndefined(prefix)) prefix = settings.prefix;
    if (_.isUndefined(key) || !_.has(Matrix, 'deviceSecret')) {
      //TODO Too much?
      module.exports.init();
      if (_.isUndefined(key) || !_.has(Matrix, 'deviceSecret')) throw 'Cypher missing device details, unable to continue';
    }
