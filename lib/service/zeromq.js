...
module.exports = {
  ...
  registerComponent: function(driver){
    ...
    if ( _.has(driver, 'send') || _.has(driver, 'prepare') || _.has(driver, 'config') ){
      o.send = config.bindSync('tcp://127.0.0.1:' + Matrix.device.port.get(driver.name).input);
    ...
    if (_.has(driver, 'ping')){
      o.ping = ping.bindSync('tcp://127.0.0.1:' + Matrix.device.port.get(driver.name).ping);
    }
