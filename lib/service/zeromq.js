...
module.exports = {
  ...
  init: function(){
    ...
    // imuConfig.connect('tcp://127.0.0.1:' + Matrix.device.port.get('gyroscope').config );
    // imuError.connect('tcp://127.0.0.1:' + Matrix.device.port.get('gyroscope').error );
    // imuUpdate.connect('tcp://127.0.0.1:' + Matrix.device.port.get('gyroscope').out );
    // imuPing.connect('tcp://127.0.0.1:' + Matrix.device.port.get('gyroscope').ping );
    //
  ...
  // Pass in Matrix.device.drivers.... export
  // Returns zmq connections
  registerComponent: function(driver){
    debug('registerComponent', driver);
    // use method / feature detection
    var send = zmq.socket('push');
    var error = zmq.socket('sub');
    var read = zmq.socket('sub');
    var ping = zmq.socket('push');
    var o = {};

    if ( _.has(driver, 'prepare')){
      send = send.connect('tcp://127.0.0.1:' + Matrix.device.port.get(driver.name).send);
      o.send = send;
    }
    ...
    if ( _.has(driver, 'read')){
      read = read.connect('tcp://127.0.0.1:' + Matrix.device.port.get(driver.name).read);
      o.read = read;
    }
