...
module.exports = {
  ...
  prepare: function(options, cb){
    ...
    debug('gesture start', options.enumName)
  ...
  config: function(gesture){

    debug('configure gesture>', gestures);

    var config = new matrixMalosBuilder.DriverConfig;
    config.malos_eye_config = new matrixMalosBuilder.MalosEyeConfig;

    // send config
    config.malos_eye_config.object_to_detect.push(matrixMalosBuilder.EnumMalosEyeDetectionType[gesture]);
