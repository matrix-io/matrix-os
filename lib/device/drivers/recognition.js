...
function getAllTags(cb) {
  ...
  RecognitionRPCService.getFeatureDescriptorTags({}, headers, function (err, res) {
...
function startRecognitionService() {
  ...
  // just in case if this is called before init
  if (_.isUndefined(RecognitionRPC)) {
    module.exports.init();
  }

  // add token to grpc headers for auth
  headers = new grpc.Metadata();
  headers.add('authorization', 'Bearer ' + Matrix.deviceToken);
  var credentials = grpc.credentials.createSsl();

  // we call this function many times, don't reinit the GRPC if it's already made
  if (_.isUndefined(RecognitionRPCService)) {
    var grpcClient = grpc.Client(HOST_ADDRESS, credentials);

    RecognitionRPCService = RecognitionRPC.create(function rpcImpl(fn, msg, cb) {
      log(fn, RecognitionService.decode(msg), RecognitionRPCService, grpc)
      cb('HI>>>')
      grpcClient.makeUnaryRequest('GET',
        function () { log('ser', arguments) }, function () { log('deser') }, headers, function () { log('cb', arguments) })
    })
...
module.exports = {
  ...
  init: function () {
    ...
    RecognitionRPC = Matrix.service.grpc.recognition.v1.recognitionService;
