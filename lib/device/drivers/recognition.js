...
module.exports = {
  ...
  config: function(){
    ...
    RecognitionService.getFeatureDescriptors({}, headers, function(err, data){
      if(err) console.error(err);
      console.log('RECOG TAGS>', data);
    })
      ...
      _.each( r.facial_recognition, function( recog ){
        ...
        if ( recog.tag === 'FACE_DESCRIPTOR') {
          ...
          if ( mode === modes.indexOf('TRAIN') ){
            ...
            if ( dCount >= 7) {
              // debug('face_desc', recog)
              debug('>f_r', recog.tag, tId );
              ...
              debug('Send to RS Training >>> ', descriptors);
