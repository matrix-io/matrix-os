...
var N_DESCRIPTORS = 3;
var RECOG_SERVICE_DEV = 'dev-recognize.matrix.one:443';
...
function startRecognitionService() {
  ...
  if (_.isUndefined(RecognitionService)) {
    ...
    debug('RecognitionService:'.blue, HOST_ADDRESS);
  }
...
function deleteTags(tags, cb) {
  ...
  RecognitionService.deleteFeatureDescriptors({
    tags: tags
  }, headers, function deletedTags(err, resp) {
...
module.exports = {
  ...
  prepare: function(component, cb) {
    ...
    if (options.hasOwnProperty('mode')) {
      ...
      if (m !== -1) {
        ...
        console.warn('Invalid Recognition Mode:', options.mode);
      }
  ...
  // unlike other reads, here we're waiting for a variety of responses
  read: function(buffer, cb) {
    // debug('read>', buffer);
    // don't continue if off or if stopped
    if (_.isUndefined(buffer) || mode === 0) {
      return;
    }
