...
module.exports = {
  ...
  read: function(buffer, cb){
    ...
    if ( result.vision_event.length > 0){
      _.map( result.vision_event, function(v) {
      ...
      if ( mode === modes.indexOf('TRAIN') ){
        ...
        _.map( r.facial_recognition, function(recog){
          if ( recog.tag === 'FACE_DESCRIPTOR'){
            ...
            debug('Demographic Info in Recog:', recog.tag);
          }
        });
        // End Map
      } else {
        debug('Read with no Mode selected');
      }
    })
