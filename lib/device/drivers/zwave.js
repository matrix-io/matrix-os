...
module.exports = {
  ...
  init: function() {
    ...
    ZW = function() {
      ...
      var cmds = {};
      cmds.msg = new matrixMalosBuilder.ZWaveMsg;
      cmds.cmd = new matrixMalosBuilder.ZWaveMsg.ZWaveCommand;

      


      cmds.msg.set_operation(enums.operations.SEND);
      cmds.msg.set_service_to_send('Switch Multilevel [f654efe50600]');

      var zWaveCmd = new matrixMalosBuilder.ZWaveMsg.ZWaveCommand;
      zWaveCmd.set_zwclass(enums.classes.COMMAND_CLASS_SWITCH_MULTILEVEL);
      zWaveCmd.set_cmd(enums.commands.SWITCH_MULTILEVEL_SET);
      zWaveCmd.set_params(new Buffer(255));
      cmds.msg.set_zwave_cmd(zWaveCmd);

      console.log('zWaveCmd:', zWaveCmd);
      console.log('cmds.msg:', cmds.msg);

      config.set_zwave(cmds.msg);
      ...
      // enums need to be referenced outside of ZW
      enums.operations = matrixMalosBuilder.ZWaveMsg.ZWaveOperations;
      enums.classes = matrixMalosBuilder.ZWaveClassType;
      enums.commands = matrixMalosBuilder.ZWaveCmdType;
      ...
      //Private ZWave Methods
      
      //#1      
      
      const __cmd = function () {
        config.zigbee_message = cmds.msg;
        if (activated && self.errors.length === 0) {
          debug(require('util').inspect(config, { depth: null }));
          Matrix.components.zwave.print(config.encode().toBuffer());
        } else if (self.errors.length > 0) {
          console.error('Z-Wave Command Errors:\n'.red, '->', self.errors.join('\n -> '.red));
        } else {
          console.error('Z-Wave Command Print Failed due to Component Inactivation');
        }
        // reset command state
        Cmd = new ZB();
      };
