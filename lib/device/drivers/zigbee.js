...
var wrapMessage = function( zbCmd ){
  config.base.zigbee_message = zbCmd;
}

var wrapNetMessage = function( netMsg ){
  cmds.msg.type = enums.zbCmdTypes.NETWORK_MGMT;
  cmds.msg.network_mgmt_cmd = netMsg
}

var wrapZCLMessage = function( msg ){
  cmds.msg.type = enums.zbCmdTypes.ZCL;
  cmds.msg.zcl_cmd = msg
}

var wrapZLLMessage = function( msg ){
  cmds.msg.type = enums.zbCmdTypes.ZLL;
  cmds.msg.zll = msg
}
...
read: function(buffer){
  ...
  if (zm.type === enums.zbCmdType.ZCL){
    debug('ZCL>', zm);
  } else if (zm.type === enums.zbCmdType.ZCL){
    debug('ZLL>', zm);
  } else if (enums.zbCmdType.NETWORK_MGMT){
    debug('NET>', zm);
    var netCmdType = zbMsg.network_mgmt_cmd.type;
    console.log(netCmdType);
  } else {
